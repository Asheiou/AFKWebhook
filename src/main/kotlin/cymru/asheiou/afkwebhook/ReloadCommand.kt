...
class ReloadCommand(val plugin: JavaPlugin) : CommandExecutor {
  override fun onCommand(
    ...
    if (webhook == null) {
      MessageSender.sendMessage(sender, "No webhook was found in your config! Please check it and try " +
              "again.")
      return true
    }
    val uri: URI
    try {
     uri = URI(webhook)
      ...
      MessageSender.sendMessage(sender, "Your webhook URI isn't in a valid link format. Please check " +
    ...
    MessageSender.sendMessage(sender, "Webhook send failed! Code: " + response.statusCode()
